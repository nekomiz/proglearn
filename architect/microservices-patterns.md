...
- [Почему важно применять шаблоны проектирования в микросервисной архитектуре?](#почему-важно-применять-шаблоны-проектирования-в-микросервисной-архитектуре)
...
- [Паттерны развертывания микросервисов](#паттерны-развертывания-микросервисов)
    1. [Шаблон «Экземпляр сервиса на хост» (Service Instance Per Host)](#1-шаблон-экземпляр-сервиса-на-хост-service-instance-per-host)
    2. [Шаблон «Сине-зеленое развертывание» (Blue-Green Deployment)](#2-шаблон-сине-зеленое-развертывание-blue-green-deployment)
- [Паттерны повышения отказоустойчивости](#паттерны-повышения-отказоустойчивости)
    1. [Шаблон «Автоматический выключатель» (Circuit Breaker)](#1-шаблон-автоматический-выключатель-circuit-breaker)
    2. [Шаблон «Переборка» (Bulkhead)](#2-шаблон-переборка-bulkhead)
- [Паттерны мониторинга микросервисов](#паттерны-мониторинга-микросервисов)
    1. [Шаблон «Агрегация логов» (Log Aggregation)](#1-шаблон-агрегация-логов-log-aggregation)
    2. [Шаблон «Распределенная трассировка» (Distributed Tracing)](#2-шаблон-распределенная-трассировка-distributed-tracing)
    3. [Шаблон «Проверки здоровья» (Health Check)](#3-шаблон-проверки-здоровья-health-check)
- [Прочие паттерны проектирования микросервисов](#прочие-паттерны-проектирования-микросервисов)
    1. [Шаблон «Посредник» («Посол», Ambassador)](#1-шаблон-посредник-посол-ambassador)
    2. [Шаблон «Коляска» («Прицеп», Sidecar)](#2-шаблон-коляска-прицеп-sidecar)
    3. [Шаблон «Тестирование контрактов, ориентированных на потребителя» (Consumer-Driven Contract Testing)](#3-шаблон-тестирование-контрактов-ориентированных-на-потребителя-consumer-driven-contract-testing)
    4. [Шаблон «Внешняя конфигурация» (External Configuration)](#4-шаблон-внешняя-конфигурация-external-configuration)
...
![alt text](ServiceInstancePerHost.png)
...
![alt text](Blue-GreenDeployment.png)
...
- __Closed__. Идет передача запросов между сервисами и подсчет количества сбоев. Если число сбоев за заданный интервал времени превышает пороговое значение, выключатель Proxy-сервера переводится в состояние Open.
- __Open__. Запросы от исходного сервиса немедленно возвращаются с ошибкой. По истечении заданного тайм-аута выключатель переводится в состояние Half-Open.
- __Half-Open__. Выключатель пропускает ограниченное количество запросов от исходного сервиса и подсчитывает число успешных запросов. Если необходимое количество достигнуто, выключатель переходит в состояние Closed, если нет — возвращается в статус Open.
...
![alt text](BulkheadResourceAllocation.png)
...
![alt text](BulkheadServiceInstancePerClient.png)
...
![alt text](DistributedTracing.png)
...
![alt text](Consumer-DrivenContractTesting.png)
...
![alt text](ExternalConfiguration.png)